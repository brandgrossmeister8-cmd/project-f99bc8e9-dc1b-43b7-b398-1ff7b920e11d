...
import { ArrowLeft, ArrowRight, Plus, Lightbulb, Trash2 } from 'lucide-react';
...
const GamePlay = () => {
  ...
  const progress = Math.min(totalPoints / MIN_POINTS, 1);
      ...
      <header className="border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display font-bold text-foreground text-sm">{currentGame.businessName}</h1>
              <p className="text-xs text-muted-foreground">{currentGame.businessProblem}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Badge variant={canProceed ? 'default' : 'secondary'} className={`font-mono ${canProceed ? 'bg-chess-dark text-chess-light' : ''}`}>
              {totalPoints}/{MIN_POINTS}
            </Badge>
            <Button size="sm" disabled={!canProceed} onClick={handleNext} className="gap-1 bg-chess-dark text-chess-light hover:bg-chess-dark/90 font-display">
        ...
        <div className="h-1 bg-muted">
          <div
            ...
            style={{
              width: `${progress * 100}%`,
              background: `repeating-linear-gradient(90deg, hsl(var(--chess-dark)) 0px, hsl(var(--chess-dark)) 8px, hsl(var(--chess-gold)) 8px, hsl(var(--chess-gold)) 16px)`,
            }}
          />
        </div>
      </header>
            ...
            <div key={stage.id} className="flex flex-col">
              <div className="flex items-center justify-between mb-2 pb-2 border-b border-border">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-display font-semibold text-foreground">{stage.name}</span>
                </div>
                <div className="flex items-center gap-1">
                  <Badge variant="outline" className="text-xs font-mono border-chess-gold/30">{points.length}</Badge>
                  {hints[stage.id] && (
                    <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setShowHints(showHints === stage.id ? null : stage.id)}>
                      <Lightbulb className="w-3 h-3 text-chess-gold" />
                    </Button>
                  )}
                </div>
              </div>
              ...
              {showHints === stage.id && hints[stage.id] && (
                <Card className="p-2 mb-2 border-chess-gold/30 bg-chess-gold-soft">
                  <p className="text-xs font-display font-semibold text-foreground mb-1">Подсказки:</p>
                  <div className="flex flex-wrap gap-1">
                    {hints[stage.id].map((h, i) => (
                      <button
                        key={i}
                        className="text-xs bg-card px-2 py-0.5 border border-border hover:border-chess-gold transition-colors text-foreground"
              ...
              <div className="flex gap-1 mb-2">
                ...
                />
                <Button size="icon" variant="outline" className="h-8 w-8 flex-shrink-0 hover:border-chess-gold" onClick={() => handleAdd(stage.id)}>
              ...
              <div className="space-y-1 flex-1 overflow-y-auto max-h-[60vh]">
                {points.map((point, idx) => (
                  <Card key={point.id} className={`p-2 flex items-center justify-between group animate-scale-in ${idx % 2 === 0 ? 'bg-card' : 'bg-muted/30'}`}>

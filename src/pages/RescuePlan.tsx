...
import { getTop3Critical, getZoneLabel, JOURNEY_STAGES } from '@/types/game';
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-foreground text-sm">План спасения</h1>
          </div>
      ...
      <div className="container py-6 max-w-3xl space-y-4">
        ...
        {top3.map((point, idx) => {
          ...
          return (
            <Card key={point.id} className="p-5 animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-semibold text-foreground">{point.name}</h3>
                  <p className="text-xs text-muted-foreground">{stageName} · {point.totalScore}/8 баллов</p>
                </div>
                <Badge style={{
                  backgroundColor: point.zone === 'red' ? '#DC3545' : '#FD7E14',
                  color: '#fff',
                }}>{getZoneLabel(point.zone)}</Badge>
              </div>

              {sampleSolution && (
                <div className="flex items-start gap-2 p-3 rounded-lg bg-zone-yellow/30 mb-4">
                  <Lightbulb className="w-4 h-4 text-zone-yellow flex-shrink-0 mt-0.5" />
